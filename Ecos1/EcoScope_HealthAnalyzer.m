function varargout = EcoScope_HealthAnalyzer(varargin)
...
% Last Modified by GUIDE v2.5 01-Jun-2015 16:14:01

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @EcoScope_HealthAnalyzer_OpeningFcn, ...
    'gui_OutputFcn',  @EcoScope_HealthAnalyzer_OutputFcn, ...
    'gui_LayoutFcn',  [], ...
    'gui_Callback',   []);
...
function EcoScope_HealthAnalyzer_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for EcoScope_HealthAnalyzer
handles.output = hObject;


%set(handles.PHM_Analysis,'Enable','inactive');
...
function varargout = EcoScope_HealthAnalyzer_OutputFcn(hObject, eventdata, handles)
...
function Run_Analysis(handles,FileInfo,Info)
%set(handles.PHM_Analysis,'Enable','off');
...
EcoScope_SubsystemAnalysis(FileInfo,Info,csvFileName);
...
if isdeployed
    ...
    if exist('TFS_code','dir')~=7
        ...
        cd([ctfroot,'\TFS_code','\Analytics','\EcoScope']);
    end
    delete('*.csv');
    delete('*.dat');
    delete('*.xls');
...
[binFileName, binPathName] = uigetfile('D:\tooldata\*.*','Select a BIN/CSV file to load','MultiSelect','off');
...
if size(binFileName,2) > 0 && ischar(binPathName),
    ...
    load_wait = waitbar(0,sprintf('Loading file. It may take 5-10 minutes. Please wait...')); % Create the waitbar
    waitbar(1/10);
    if ~isempty(regexpi(binFileName, '.BIN'))
        ...
        %path = ['PHMConverter.exe' ' "' binPathName binFileName '" ' ' "' cd '"'];
        ...
        datFileName = binFileName;  %% modified to use original bin file name for results
...
%% remove first 2 min after tool start
%removeTimeIdx=[];
...
channelData(removeTimeIdx, :)= [];
   ...
   % EnvChannelProcess(channelData,channelName);
    ...
    for n=1:length(shock_channels)
        ...
        if isfield(shockchecks.(shock_channels{n,1}),'OutOfRangeTime') == 1
   ...
   limitCheckData = [limitcheck_channels, status',condition',minimum',maximum',failurePercentage', outagecount', outageTime'];
    clear status condition minimum maximum failurePercentage;
...
if exist('EcoScope_TechlogChannel_Data.mat','file') == 2
    ...
    if validfile==1  %|| ~isdeployed
        Run_Analysis(handles,FileInfo,Info);%run analysis
    end
end
if exist('Limit_Check_Data.mat','file')==2
    delete('Limit_Check_Data.mat');
