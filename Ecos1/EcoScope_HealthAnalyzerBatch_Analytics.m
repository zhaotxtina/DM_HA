...
%         FileNames = cell(length(files),1);
        ...
        for k = 1:length(files)
            ...
            if size(binFileName,2) > 0 && ischar(binPathName),
                ...
                if ~isempty(strfind(binFileName, '.BIN'))
                    ...
                    datFileName = strrep(strrep(binFileName,'.bin',''),'.BIN','');  %% modified to use original bin file name for results
                ...
                elseif ~isempty(strfind(binFileName, '.CSV'))
               ...
               % EnvChannelProcess(channelData,channelName);
                ...
                shockfilename = repmat({strrep(datFileName,'.CSV','')},length(shock_channels),1);
                ...
                limitfilename = repmat({strrep(datFileName,'.CSV','')},length(limitcheck_channels),1);
                totalrecords = length(channelData);
                ...
                limitCheckData = [limitcheck_channels, status',condition',minimum',maximum',failurePercentage', outagecount'];
                limitCheckBatchData = [limitfilename,limitcheck_channels, status',condition',minimum',maximum',failurePercentage',outagecount'];
                
                %TotalLimitCheckData = [TotalLimitCheckData;limitCheckBatchData];
                
                clear status condition minimum maximum failurePercentage outagecount;
                ...
                statusWordsfilename = repmat({strrep(datFileName,'.CSV','')},length(names),1);
                statusWordsCheckData = [names',status',description',condition',activationPercentage'];
                statusWordsCheckBatchData = [statusWordsfilename,names',status',description',condition',activationPercentage'];
                %TotalStatusWordData = [TotalStatusWordData; statusWordsCheckBatchData];
                save('StatusWords_Check_Data','statusWordsCheckData');
                clear names status description condition activationPercentage statusWordsCheckData;
                ...
                data_struct.filename =  strrep(datFileName,'.CSV','');
 %               data_struct.filetime = datFileTime;
            ...
            if exist('EcoScope_TechlogChannel_Data.mat','file') == 2
                ...
                if validfile==1  %|| ~isdeployed
                    ...
                    FileNames= strrep(datFileName,'.CSV','');
...
TechlogResultfilename = strcat(strrep(csvFileName,'.CSV',''),'_BatchTechlogResults.xls');

totalStatusHeader = {'File Name', 'Overall Status', 'Limit Checks', 'Environmental', 'Status Words'};

limitcheckheader = {'File Name', 'Channel Name','Status','Condition','Minimum','Maximum','Outage %','Outage Counts'};

Environmentalheader = {'File Name', 'Channel Name','Status','Condition','Maximum','Time Over Limit (min)','Outage %'};

StatusWordsheader = {'File Name','Channel Name','Status','Description','Condition','Activation %'};
